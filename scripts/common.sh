...
function get_url_travis_build_status() {
    ...
    echo "https://travis-ci.org/devopshq/youtrack.svg?branch=${BRANCH_URL_ENCODE}"
}
...
function git_set_tag() {
    ...
    git tag -a "$TAG_VALUE" -m"build log https://travis-ci.org/devopshq/youtrack/builds/${BUILD_ID}"
}
